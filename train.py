...
# from myEnv import ServoControlEnv
...
class TrainConfig:
    # Experiment
    device: str = "cuda"
    env: str = "ServoControl-v0"
    # env: str = "halfcheetah-medium-expert-v2" # OpenAI gym environment name
    seed: int = 0  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = int(5e4)  # How often (time steps) we evaluate
    n_episodes: int = 10  # How many episodes run during evaluation
    max_timesteps: int = int(5e6)  # Max time steps to run environment
    checkpoints_path: Optional[str] = "/workspace/CORL/weights/"  # Save path
    load_model: str = ""
...
def eval_actor(
    ...
    for i in range(n_episodes):
        ...
        while not done:
            if not env.x_done:
                episode.append(state[0] * 640)
            else:
                episode.append(state[1] * 480)

            action = actor.act(state, device)
            state, reward, done, info = env.step(action[0])
            episode_reward += reward

            if not before_x_done and env.x_done:
                episode.append(state[0] * 640)

            before_x_done = env.x_done
            if done:
                episode.append(state[1] * 480)

        states.append(episode)
        episode_rewards.append(episode_reward)

    actor.train()
    return np.asarray(episode_rewards), states
...
def createDataset(env):
    ...
    while total_nums <= 1000000:
        state, terminal = env.reset(), False

        while not terminal:
            total_nums += 1
            random_rate = total_nums / 1100000
            random_rate = 0.9 if random_rate >= 0.9 else random_rate
            sample = np.random.uniform(0, 1)
            noisy = 0
            if sample < random_rate:
                noisy = np.random.uniform(-0.15, 0.15)

            observations.append(state)
            current_pos = state[0] * 640
            next_target = 0 if env.current_point_ind == -1 else env.points[env.current_point_ind]

            if abs(next_target - current_pos) <= 2:
                action = 1 + noisy
            else:
                if current_pos < env.points[env.current_point_ind + 1]:
                    dist = env.points[env.current_point_ind + 1] - current_pos
                    dist_nomalized = dist / 160
                    dist_nomalized = 0.15 if dist_nomalized > 0.15 else dist_nomalized
                    action = np.random.uniform(0, 2 * dist_nomalized)
                elif current_pos > env.points[env.current_point_ind + 1]:
                    dist = env.points[env.current_point_ind + 1] - current_pos
                    dist_nomalized = dist / 160
                    dist_nomalized = -0.15 if dist_nomalized < -0.15 else dist_nomalized
                    action = np.random.uniform(2 * dist_nomalized, 0)
...
def read_dataset(data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        ...
        for data_line in all_data:
            ...
            obs_list = data[0].split(',')
            new_obs = []
            for obs in obs_list:
                if obs[0] == '[':
                    obs = obs.split('[')
                    new_obs.append(float(obs[-1]))
                elif obs[-1] == ']':
                    obs = obs.split(']')
                    if obs[0] != '':
                        new_obs.append(float(obs[0]))
                else:
                    new_obs.append(float(obs))
            observations.append(np.array(new_obs, dtype=np.float32))

            action = (data[1].split('['))[-1]
            action = (action.split(']'))[0]
            actions.append(np.array([action], dtype=np.float32))

            obs_next_list = data[2].split(',')
            new_next_obs = []
            for next_obs in obs_next_list:
                if next_obs[0] == '[':
                    next_obs = next_obs.split('[')
                    new_next_obs.append(float(next_obs[-1]))
                elif next_obs[-1] == ']':
                    next_obs = next_obs.split(']')
                    if next_obs[0] != '':
                        new_next_obs.append(float(next_obs[0]))
                else:
                    new_next_obs.append(float(next_obs))
            next_observations.append(np.array(new_next_obs, dtype=np.float32))

            rewards.append(np.float32(data[3]))

            terminal = data[4].strip()
            if terminal == 'False':
                terminals.append(int(0))
            else:
                terminals.append(int(1))
...
def draw_pyplot(states, title, path):
    for state in states:
        total_steps = [i for i in range(len(state))]
        plt.plot(total_steps, state)
        plt.title(title)
        plt.xlabel("Step")
        plt.ylabel("States")
...
def train(config: TrainConfig):
    ...
    my_dataset = read_dataset(r'./train_data/data_offline.txt')

    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]

    dataset = d4rl.qlearning_dataset(env, dataset=my_dataset)
    ...
    print("observations:", dataset["observations"])
    print("actions:", dataset["actions"])
    print("next_observations:", dataset["next_observations"])
    print("rewards:", dataset["rewards"])
    print("terminals:", dataset["terminals"])
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            with open(r"/workspace/CORL/weights/cumulativeReward.txt", 'a') as f:
                f.write(str(eval_score) + ':')

            normalized_eval_score = env.get_normalized_score(eval_score) * 100.0
            mean_eval_score += (0.85*normalized_eval_score)
            ...
            if ((t + 1) % config.eval_freq * 1) == 0:
                if config.checkpoints_path:
                    torch.save(
                        trainer.state_dict(),
                        os.path.join(config.checkpoints_path, f"checkpoint_{t + 1}.pt"),
                    )
                if normalized_eval_score >= mean_eval_score:
                    torch.save(
                        trainer.state_dict(),
            ...
            counter += 1
            title = f"State Trajectory {counter}"
            draw_pyplot(states, title, r"/workspace/CORL/weights/output")
